...
// Wraps the shared SystemHandler with NATS-specific manifest config.
type SystemHandler = commonapi.SystemHandler

// NewSystemHandler creates a new SystemHandler with NATS-specific manifest.
func NewSystemHandler(backend core.Backend) *SystemHandler {
	return commonapi.NewSystemHandler(backend, commonapi.ManifestConfig{
		ImplementationName: "ojs-backend-nats",
